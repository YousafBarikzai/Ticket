...
import { authorisationUrl, challengeFor, createVerifier, endSessionUrl, exchangeCode, readClaims } from '../bff/oidc.js';
import { ConfigurationError, developmentSignInAvailable, readConfig } from '../bff/config.js';
...
describe('configuration', () => {
  it('refuses to run in production without an identity provider', () => {
    expect(() => readConfig({ NODE_ENV: 'production' })).toThrow(ConfigurationError);
  });

  it('names the missing setting rather than failing on the first request', () => {
    expect(() =>
      readConfig({ NODE_ENV: 'production', OIDC_ISSUER: 'https://id.example.test' }),
    ).toThrow(/OIDC_CLIENT_ID/);
  });

  it('offers the development sign-in only with no provider and outside production', () => {
    expect(developmentSignInAvailable(readConfig({ NODE_ENV: 'development' }))).toBe(true);
    expect(
  ...
  it('trims a trailing slash so two configurations do not produce two URLs', () => {
    const config = readConfig({ API_BASE_URL: 'http://api.test/', WORKBENCH_ORIGIN: 'https://desk.test/' });
