...
/**
 * AI triage on a ticket (ADR-0051, `suggest` and `auto` modes).
 *
 ...
 *
 * In `auto`, a category or team the AI set by itself is shown as set by AI,
 * with Undo: the agent's own edit putting back what was there, recorded as a
 * correction. Enough corrections and the desk steps back to `suggest`.
 *
...
export function TriageCard({ triage, ticketVersion, canAct }: TriageCardProps): ReactNode {
  ...
  const applied = orderApplied(triage.applied ?? []).filter((item) => !done.has(item.question));
  if (items.length === 0 && applied.length === 0) return null;

  async function respond(question: string, response: 'accept' | 'dismiss' | 'undo'): Promise<void> {
    setBusy(question);
    setError(null);
    try {
      if (response === 'accept') await api.acceptTriage(triage.decisionId, question, ticketVersion);
      else if (response === 'undo') await api.undoTriage(triage.decisionId, question, ticketVersion);
      else await api.dismissTriage(triage.decisionId, question);
      setDone((previous) => new Set([...previous, question]));
      // Accept and Undo change the ticket, and the version with it: read the
      // page again so the next action carries the new one.
      if (response !== 'dismiss') router.refresh();
    } catch (failure) {
      setError(
  ...
  return (
    <section className="itsm-Triage" aria-labelledby="triage-heading">
      <h2 id="triage-heading" className="itsm-Triage__heading">
        {applied.length > 0 ? 'AI triage' : 'Suggested triage'}
      </h2>
      {applied.length > 0 ? (
        <>
          <p className="itsm-Triage__note">Set by AI when this ticket arrived. Undo it if it is wrong.</p>
          <ul className="itsm-Triage__list">
            {applied.map((item) => (
              <li key={item.question} className="itsm-Triage__item" data-kind="applied">
                <p className="itsm-Triage__field">
                  <strong>{fieldName(item.question)}</strong>{' '}
                  <Badge intent="info" srPrefix="Changed by">
                    Set by AI
                  </Badge>
                </p>
                <p className="itsm-Triage__line">{appliedLine(item)}</p>
                {canAct ? (
                  <div className="itsm-Triage__actions">
                    <Button
                      size="sm"
                      variant="secondary"
                      onClick={() => void respond(item.question, 'undo')}
                      disabled={busy !== null}
                      aria-label={`Undo the ${fieldName(item.question).toLowerCase()} the AI set: ${item.display}`}
                    >
                      Undo
                    </Button>
                  </div>
