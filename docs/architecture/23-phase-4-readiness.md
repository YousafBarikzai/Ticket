...
| A test fixture read as a real credential | `Bearer sk_live_notarealkey`, written as a redaction-test fixture, failed the secret scanner. That is the scanner working: it cannot tell a fixture from a leak, and neither can a person skimming a diff. A test fixture should never be shaped like a credential. |
| The first allowlist entry for it was too broad | Written without the closing quote, it would also have exempted `sk_live_notarealkeyBUTLONGER` — an unanchored literal is a prefix anybody can append a real key to. Caught by re-running the Phase 2 verification: assert the exemptions hold, then assert that real credentials wearing the same clothes are still reported. The lesson repeats because the mistake is easy. |
