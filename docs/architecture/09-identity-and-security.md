...
- **JIT provisioning:** on first login the platform's `UserService.provisionFromToken` creates the user (email, name, IdP subject) and maps group claims to teams by tenant-configured rules; subsequent logins update changed attributes. SCIM (`/scim/v2`, ADR-0037) is the preferred source and JIT falls back to it: a first login whose token names nobody links to the SCIM-made account by email.
...
- **Sessions:** Keycloak holds the SSO session; the platform records `session (user_id, sid, device, ip, ua, last_seen, expires_at)` on login and enforces tenant idle/absolute timeouts by refusing refresh beyond them and denylisting `sid` in Redis on revoke (`DELETE /me/sessions/{id}`, admin revoke, SCIM deactivate, break-glass). Three details decide whether that is a control or a description of one (ADR-0044):
  - The denylist key is `sess:deny:<sid>`, written by `revokeSession` and `deactivateUser` **inside** the transaction that revokes the row. A rollback then leaves a deny entry for a session that was not revoked — somebody signed out who did not need to be, which is the safe direction. The other order commits a revocation that never takes effect.
  - A Redis outage **fails the revocation** with `DependencyUnavailableError` → `503`. The verifier already fails *open* on the same outage, so the two are consistent: while Redis is down this platform cannot enforce revocation and says so, rather than telling an administrator that access has been withdrawn when it has not.
  - The key is global, not tenant-prefixed, and the isolation suite's keyspace scan exempts it by name. A `sid` is unique across tenants, and the verifier consults the list before it knows which tenant the token names. The key holds no tenant data: the key is the session identifier, the value is `1`, and the TTL is the session's expiry plus an hour of clock skew.
- **Recording is best-effort and self-healing.** The BFF calls `POST /api/v1/auth/session` from `createSession` — the one place both sign-in paths pass through — and again on every token refresh. A failure there is logged and does **not** fail the sign-in: refusing entry because one table is unreachable converts a degraded capability into a total outage at the moment a person has no other route in. The cost is that an unrecorded session cannot be listed or revoked until the next refresh, which is the bound on the window.
...
| Flow | Steps |
|---|---|
| Web login | App redirects to Keycloak `/auth` with `kc_idp_hint` or organisation scope derived from the tenant subdomain → identity-first login → IdP → Keycloak issues code → Next.js BFF exchanges code (PKCE) → stores tokens in Redis session → sets `__Host-session` cookie → calls `POST /api/v1/auth/session` so the platform has a session to list and revoke. JIT is not done here: `apps/api/src/plugins/context.ts` provisions from the token on the first authenticated request, which covers every caller rather than only the ones that remember to announce themselves. |
| Mobile login | `expo-auth-session` PKCE against Keycloak → tokens in secure store → `POST /api/v1/auth/session`. Biometric unlock *(PH-4)* gates local token use. |
