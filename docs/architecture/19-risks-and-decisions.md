...
| OD-04 | AI providers and residency | **Closed** — Anthropic (ADR-0042) | The adapter is registered by `AI_PROVIDER=anthropic` with a key; the stub throws at boot in production. Closing it changed no governance code, which was the bet ADR-0040 made. Two things it did not settle: model prices are operator-supplied (`AI_MODEL_PRICES`, micro-pence per thousand tokens) because a vendor's price is not this repository's to assert — and an unpriced model is now refused rather than costed at zero. **Residency is now closed** (ADR-0047): `tenant.ai_allowed_regions` states where a tenant's prompts may be processed, a provider declares where it processes, and the gateway refuses a call outside the list rather than falling back to another provider. Empty means "wherever this tenant's own data lives". The check runs before the prompt is rendered, so a forbidden call never interpolates ticket text at all. | PH-5 |
| OD-05 | Commercial model | **Closed:** per-agent seats, three tiers (product owner, Sept 2026) | Starter £19, Professional £45, Enterprise negotiated, plus an unpriced `trial` that is not a tier. Per agent per month, in micro-pence on the `plan` row, returned as a string over the API. Enterprise has no list price on purpose: at that size the number comes from a negotiation. **Nothing charges anybody** — there is no payment provider, invoice, proration or dunning, and doc 23 carries that as absent (ADR-0047) | — |
| OD-06 | Product name, domain, branding | **Closed:** Railway, as docs 03 and 16 already assumed (product owner, Sept 2026) | `infra/railway` already exists. The domain layout is unchanged — `help`, `desk`, `admin`, `api`, `auth`, `status` plus a wildcard for tenants (16 §2) — and is parameterised rather than hard-coded, so a real domain replaces a placeholder without a code change. The deploy pipeline itself (stages 4–6 of 16 §3) is now built: `deploy.yml`, `infra/railway/services.json`, `infra/keycloak/realm.json` and the two scripts that apply them (ADR-0050). Every deploy job is gated on `vars.DEPLOY_DOMAIN` and `secrets.RAILWAY_TOKEN`, so until they are set the pipeline builds, scans and pushes the images and prints the deploy plan it would have run | PH-5 |
| OD-07 | Time tracking default | **Closed:** not billable unless somebody says so (product owner, Sept 2026) | Schema default `false`, and no seeded activity type is billable. Under-billing is a conversation somebody has; invoicing a customer for work nobody meant to charge for is a refund and a lost argument about trust. Existing entries are left alone — the migration changes the default, not the rows (ADR-0047) | — |
