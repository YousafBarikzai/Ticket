...
| OD-06 | Product name, domain, branding | **Closed:** Railway, as docs 03 and 16 already assumed (product owner, Sept 2026) | `infra/railway` already exists. The domain layout is unchanged — `help`, `desk`, `admin`, `api`, `auth`, `status` plus a wildcard for tenants (16 §2) — and is parameterised rather than hard-coded, so a real domain replaces a placeholder without a code change. The deploy pipeline itself (stages 4–6 of 16 §3) is separate work this decision unblocks rather than performs | PH-5 |
