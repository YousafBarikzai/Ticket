...
| MOD-09 Knowledge, search, AI | `modules/knowledge`, `modules/search`, `modules/ai` | KnowledgeArticle, ArticleVersion, ArticleFeedback, DecisionTree, SearchDocument; AiPrompt, AiPromptVersion, AiEvalDataset, AiEvalCase, AiEvalRun, AiBudget, AiJob, AiSuggestion, AiDecision | `ArticleService`, `SearchService`, `Indexer`, `callModel` and `decide` (the gateway), `suggestionService`, `promptService`, `evalService`, `aiBudgetService` | `knowledge.article.*`, `ai.suggestion.created`, `ai.budget.threshold`, `ai.decision.stepped_down`, `ticket.classified` (ADR-0051) | `ticket.*`, `knowledge.*`, `catalogue.item.published`, `asset.*`, `knownerror.published` | MOD-04, MOD-01, MOD-13, MOD-15, MOD-14 |
...
| MOD-22 ESM packs | `modules/esm` (pack content in `modules/esm/src/packs`) | PackInstallation, PackItem | `packService`, `domain/pack`, `domain/diff` | `pack.installed`, `pack.upgraded` | `request.submitted` | MOD-05, MOD-06, MOD-07, MOD-09, MOD-13, MOD-01, MOD-15 |
| MOD-23 Status page | `modules/statuspage` (public page served by `apps/api`; static export deferred, ADR-0035) | StatusPage, StatusComponent, StatusIncident, StatusUpdate, MaintenanceWindow, StatusSubscriber | `pageService`, `incidentService`, `publicService` | `status.incident.updated`, `status.maintenance.scheduled` | `incident.major.declared/updated/resolved`, `change.scheduled`, `change.closed` | MOD-08-E1, MOD-08-E3, MOD-05, MOD-11, MOD-21 |
| MOD-24 Migration | `modules/migration` | ImportJob, ImportMapping, ImportFile, ImportRecord, ImportLink | `jobService`, one importer per entity | `import.job.finished` | — (writes through other modules' services; MOD-04 publishes `ticket.imported`, ADR-0036) | MOD-01, MOD-04, MOD-05, MOD-11, MOD-14, MOD-21 |
...
- **MOD-09's AI half ships no `Embedding` table and no `VirtualAgentService`.** Hybrid retrieval is lexical only: the ACL-filtered search projection answers the question, and embeddings generated by a stub would be noise (ADR-0040). The virtual agent is not built at all, because the line ADR-0006 draws — a person reviews before anything reaches a requester — is the line this phase keeps. Both wait behind OD-04. `PromptTemplate` becomes `AiPrompt` plus an immutable `AiPromptVersion`, because a prompt that could be edited in place is a prompt whose evaluation score means nothing.
- **Pack content lives inside MOD-22** (`modules/esm/src/packs`) rather than in a top-level `packs/*`, and the entities are `PackInstallation` and `PackItem` rather than `ServicePack`. A pack is written with the deployment and never uploaded (ADR-0039), so there is no `ServicePack` table for a tenant to hold: what a tenant owns after installing is ordinary catalogue, workflow, SLA and knowledge configuration, and the two tables here are the receipt that says where it came from.
