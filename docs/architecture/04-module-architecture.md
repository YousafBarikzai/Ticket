...
| MOD-08 ITIL practices | `modules/incident`, `modules/problem`, `modules/change` | MajorIncident, MajorIncidentUpdate, Problem, KnownError, Change, ChangeWindow, BlackoutWindow, StandardChangeTemplate, PostIncidentReview, ActionItem, MonitoringEvent | `MajorIncidentService`, `ProblemService`, `ChangeService`, `AlertCorrelationService` *(PH-4)* | `incident.major.*`, `problem.created`, `knownerror.published`, `change.*` | `approval.decided`, `ci.relationship.changed`, monitoring events | MOD-04, MOD-17, MOD-10, MOD-20; MOD-11 and MOD-23 by events |
...
| MOD-10 Assets and CMDB | `modules/assets`, `modules/cmdb` | CiClass, ConfigurationItem, Asset, AssetModel, CiRelationship, Contract, Warranty, Supplier, DiscoverySource, ReconciliationRule, ReconciliationRun, AssetAssignment | `CiService`, `AssetService`, `RelationshipService`, `ImpactService`, `ReconciliationService` | `asset.*`, `ci.relationship.changed`, `contract.expiring` | `user.deactivated`, `import.job.completed`, `change.implemented` | MOD-01, MOD-04, MOD-14, MOD-24 |
...
| MOD-20 Workload | `modules/workload` | Availability, Shift, ShiftAssignment, OnCallRotation, OnCallOverride, Skill, AgentSkill, RoutingPolicy | `AvailabilityService`, `AssignmentStrategy`, `ScheduleService`, `OnCallService` | `agent.availability.changed`, `shift.*`, `oncall.*` | `ticket.created/updated`, `incident.major.declared`, `sla.timer.warning` | MOD-01, MOD-04, MOD-07, MOD-11 |
